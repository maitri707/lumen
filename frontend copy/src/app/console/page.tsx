...
export default function ConsolePage() {
  ...
  // STT Toggle configuration
  const sttEngine = process.env.NEXT_PUBLIC_STT_ENGINE || "browser";
  ...
  const { isListening: isVadListening, startListening: startVad, stopListening: stopVad } = useVADRecorder((base64) => {
    wsRef.current.sendAudioBlob(base64);
  });

  const isListening = sttEngine === "groq" ? isVadListening : isBrowserListening;
  ...
  // Initialize Speech Recognition (for browser fallback mode)
  ...
  }, [sendCommand, sttEngine]);

  const toggleListening = useCallback(() => {
    if (sttEngine === "groq") {
      if (isVadListening) {
        stopVad();
        setAgentActivity("Microphone disabled.");
      } else {
        startVad();
        setAgentActivity("Listening continuously (Groq VAD)...");
      }
      return;
    }
  ...
  }, [sttEngine, isVadListening, startVad, stopVad, recognition, isBrowserListening]);
  ...
  }, [isScreenSharing, sttEngine, isVadListening, startVad, stopVad, recognition, isBrowserListening]);
